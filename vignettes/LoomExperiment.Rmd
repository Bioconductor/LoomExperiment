...
The `colGraphs` and `rowGraphs` slots correspond to the `col_graphs` and `row_graphs` fields in the Loom File format.  Both of these slots require `LoomGraphs` objects.

A `LoomGraph` class extends the `DataFrame` class from the `S4Vectors` package with the requirements that a `LoomGraph` object must:
