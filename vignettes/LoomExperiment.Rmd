...
`LoomGraph` objects also can be subset on a by-node basis.  The remaining examples in this section will explore this.  First, the `selectHits()` method allows the user to select edges based on whether a node number is present.

```{r select_LoomGraph}
selectHits(lg, c(1, 3))
```

The `dropHits()` method removes edges based on whether a node number is present.

```{r drop_LoomGraph}
dropHits(lg, c(1, 3))
```

The `dropHits()` method can also replace the given nodes with different nodes.

```{r dropreplace_LoomGraph}
dropHits(lg, 2) <- 5
lg
```
...
The `selectHits()` and `dropHits()` methods can also be used on a `LoomExperiment` object to effect all `colGraph` and `rowGraph` objects contained within the `LoomExperiment` object.

```{r selectdropHits_LoomExperiment}
